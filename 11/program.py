...
def set_color_input(coordinate):
    
    if coordinate in COLOR:
...
def update_direction(turn, direction):
    if is_left(turn):
        ...
        raise(RuntimeError("UNKNOWN turn: {}".format(turn)))

    return direction % 4

def step(coordinate, direction):
    x = coordinate.x
    y = coordinate.y
    if direction == 0:
        y = y + 1
...
def parse_program_output(coordinate, direction):
    painted = get_program_output()
    PAINTED[coordinate] = True
    ...
    if painted == black():
        ...
        raise("Unknown color: {}".format(painted))

    turn = get_program_output()
    new_direction = update_direction(turn, direction)
    print("old_direction = {}, new = {}".format(print_direction(direction), print_direction(new_direction)))
    new_coordinate = step(coordinate, new_direction)
    return new_coordinate, new_direction
